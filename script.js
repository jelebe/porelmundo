        ...
        onValue(markersRef, (snapshot) => {
            snapshot.forEach((childSnapshot) => {
                ...
                if (
                    ...
                    remove(specificRef)
                        .then(() => console.log('Marcador eliminado de Firebase'))
                        .catch((error) => console.error('Error al eliminar el marcador de Firebase: ', error));
                }
            });
        });
    }
            ...
            if (data) {
                for (let key in data) {
                    if (data.hasOwnProperty(key)) {
                        let markerData = data[key];
                        addMarker(
                            L.latLng(markerData.latlng.lat, markerData.latlng.lng),
                            markerData.image,
